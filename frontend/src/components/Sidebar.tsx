...
import { listGroups } from '@/contexts/api';
...
export const Sidebar = memo(() => {
  ...
  const handleGroupSelect = (groupId: string | null) => {
    dispatch({ type: 'SET_SELECTED_GROUP', payload: {
      id : groupId,
      name : "" // TODO : GET BACK TO EDIT
    } });
  };
  console.log("side bar Groups", state.groupList);
            ...
            {state.groupList.map((group) => (
              <Button
                ...
                className={cn(
                  "w-full justify-start text-left font-normal",
                  "text-primary-foreground hover:bg-primary/80 hover:text-car-accent",
                  state.selectedGroup.id === group.id && "bg-primary/60 text-car-accent"
                )}
